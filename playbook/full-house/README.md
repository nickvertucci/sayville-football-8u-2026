...
The full house T. Both ends are tight and on the line, and the three backs sit shoulder to shoulder four yards deep — left halfback, fullback, right halfback, evenly spaced. Nobody is offset and nobody is deeper, so the alignment gives away nothing. The formation is perfectly symmetric, so every play has a left and a right version off identical rules.
...
| [House Boot Left](#house-boot-left) | `House 17 Boot` | pass | LE |
