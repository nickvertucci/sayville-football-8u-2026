...
/* The rotations: position cards in one row, left to right in the order the file
   lists them, however many there are and however narrow the screen. They share the
   width equally rather than wrapping -- a rotation read across is the point, and
   the Z dropping to a second line on a phone lost it. */
.dc-rotrow {
...
/* Five across a phone is five columns of about seventy pixels, so the type follows
   the screen down rather than the cards dropping a row. Sized so "Michael M." still
   sits on one line in its column at 360px. */
@media (max-width: 820px) {
  .dc-rotrow { gap: 4px; }
  .dc-rot .dc-pos-h { padding: 3px 2px; }
  .dc-rot .dc-pos-h .dc-abbr { font-size: 2.5vw; letter-spacing: 0; }
  .dc-rot .dc-names li { font-size: 2.2vw; padding: 2px 2px; gap: 2px; }
...
@media print {
  ...
  /* 6.6in, not the defense's 7.4: this sheet also carries the page title above it,
     and at 7.4 the two ended 6px short of the foot of a Letter page in Chrome -- so on
     A4, a phone, or a print dialog with its own margins the rotations went to a second
     sheet. 6.6 leaves most of an inch for any of those. */
  /* Only at the paper's own width. The height is in inches and the layout is not: a
     phone that prints the page as laid out on its screen shrinks a 390px-wide column
     onto the sheet, and 6.6 inches of it is most of two pages -- the rotations pinned
     to the foot of it were what landed on the second one. Narrower than a landscape
     sheet, the rotations just follow the board. */
  @media (min-width: 900px) {
    .dc-side[data-side="offense"] {
      display: flex; flex-direction: column; min-height: 6.6in;
    }
    .dc-side[data-side="offense"] .dc-rots { margin-top: auto; }
  }
  .dc-rots .rot-h { font-size: 9pt; color: #000; }
  .dc-rot .dc-pos-h .dc-abbr { font-size: 14pt; line-height: 1.3; }
  .dc-rot .dc-names li { font-size: 15pt; line-height: 1.25; padding: 1px 6px; gap: 5px; }
  .dc-rot .dc-names li b { flex-basis: 14px; font-size: 10pt; }
  .dc-rot .dc-names li.starter { font-size: 16pt; line-height: 1.25; }
  /* A phone that prints without the Print button's widening -- from its share sheet
     -- lays this out at its own screen width, where the point sizes above overflow
     five columns. The type follows the width down instead, so the row stays one row. */
  @media (max-width: 820px) {
    .dc-rotrow { gap: 4px; }
    .dc-rot .dc-pos-h { padding: 3px 2px; }
    .dc-rot .dc-pos-h .dc-abbr { font-size: 2.5vw; letter-spacing: 0; }
...
def print_width_px(page_rule: str) -> int:
    """The width, in CSS pixels, a desktop browser lays a page out at to print it.

    A desktop browser sizes the print layout to the paper: the sheet less its margins,
    at 96 pixels an inch. A phone does not -- it lays the page out at the width of its
    own screen and shrinks that onto the paper, so every rule written for a narrow
    screen fires on paper too and a sheet built to be one page comes out as two. The
    site script reads this number off the page and widens a phone to it before it
    prints, so the phone lays out what the desktop does.

    Letter throughout, as the page rules are. A page with no rule gets the browser's
    own margins, which are close to 0.4in.
    """
    rule = page_rule or ""
    paper = 11.0 if "landscape" in rule else 8.5
    margin = 0.4
    m = re.search(r"margin:\s*([\d.]+)(in|mm)", rule)
    if m:
        margin = float(m.group(1)) / (25.4 if m.group(2) == "mm" else 1)
    return round((paper - 2 * margin) * 96)
...
def page(
    ...
    print_px = print_width_px("size: landscape; margin: 9mm;" if landscape else page_rule)
    desc = (
        f'\n<meta name="description" content="{esc(description)}">' if description else ""
    )
    return f"""<!doctype html>
<html lang="en" data-print-width="{print_px}">
<meta charset="utf-8">
