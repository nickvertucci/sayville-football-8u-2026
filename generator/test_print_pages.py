...
PRINT_BTN_MARK = 'onclick="window.print()"'
# Site chrome that must never land on paper. Header, footer, skip link and the
# Print button itself are display:none in the print stylesheet; if any of these
# strings survive into the PDF, that rule has slipped.
PRINT_CHROME = (
    "Skip to content",
    "Generated by",
    "Open menu",
...
def site_html() -> list[Path]:
    return sorted(p for p in ROOT.glob("*.html") if p.is_file())


def pdf_contains(pdf: Path, needle: str) -> bool:
    blob = pdf.read_bytes()
    return (
        needle.encode("latin-1") in blob
        or needle.encode("utf-16-be") in blob
        or needle.encode("utf-16-le") in blob
    )
...
def main(argv=None) -> int:
    ...
    failures = []
    missing_btn = [p.name for p in site_html()
                   if PRINT_BTN_MARK not in p.read_text(encoding="utf-8")]
    if missing_btn:
        failures.append("no Print button: " + ", ".join(missing_btn))
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        if not args.quick:
            for page in plays + fronts + practices:
                ...
                for leak in PRINT_CHROME:
                    if pdf_contains(tmp / f"{page.stem}.pdf", leak):
                        failures.append(f"{page.name} print still has {leak!r}")

            # The other site pages are not one-sheet cards, but they still print —
            # no nav, no skip link, no footer — and every one has a Print button.
            listing = [
                ROOT / "index.html",
                ROOT / "install.html",
                ROOT / "defense.html",
                ROOT / "rules.html",
                *sorted(ROOT.glob("f-*.html")),
            ]
            for page in listing:
                out = tmp / f"{page.stem}.pdf"
                n = render(chrome, page, out, profile)
                print(f"  {page.name:<26} {n}")
                if n < 1:
                    failures.append(f"{page.name} produced an empty print")
                for leak in PRINT_CHROME:
                    if pdf_contains(out, leak):
                        failures.append(f"{page.name} print still has {leak!r}")
